...
**ACQ-4.**
`maxTimeToIdle` bounds how long a session outlives its use: the canister ends one from which nothing has been minted for that span. It is requested as a ceiling in the same way `maxTimeToLive` is, in the same units, and clamped to the same range — 10 minutes to the granted session length. Where an application sets none, the canister's default of seven days applies, so a session nobody touches ends well inside the thirty days one may otherwise live.

Under the 8-hour default above the two bounds do not yet meet: seven days is clamped down to the granted length, and such a session expires long before it can go idle. The idle bound is what keeps that true once `maxTimeToLive` rises — a 30-day session left alone still ends after seven — which is why the default belongs here rather than arriving with that release.
...
**STATE-10.**
A state store declares whether a sign-in recorded in it can be recovered without a ceremony, and that declaration is the default for what the client tells the identity provider. It is the one fact about a state store's medium that has to leave this library, because the identity provider's frontend decides whether to keep a session and cannot see which store an application chose. It never reaches the canister.
