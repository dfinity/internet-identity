...
One outcome for every session-related case, so a client's fallback is a single branch. The `prompt=none` rules above already bound what a client can learn from it, since it only ever answers for the requesting origin: the `prompt=none` rules above already bound what it can be used to learn, since it only ever answers for the requesting origin.
...
| SEL-6 | A held session MUST be answered only when the request asks for silence. An absent `prompt`, like `prompt=login`, MUST run the ceremony, so silence is something an app opts into rather than a default. |
