...
which is the session's own principal. One index turns that into a session:

```rust
StableBTreeMap<Principal, StorableSessionHandle>

StorableSessionHandle { account_principal: Principal, device_id: SessionDeviceId }
```
...
from that, and `device_id` picks the record inside it, since a browser holds at most one
session per account ([one browser, one session per account](#one-browser-one-session-per-account)).
...
counter that has drifted high therefore costs one extra pass, never a refused sign-in; one
that had drifted low could not admit past the cap, because the recount would catch it.
...
`valid_for` is nanoseconds, clamped to between 10 minutes and 30 days. Every ceremony
...
The frontend cannot know that an app revoked a session behind its back, so it asks the canister with `check_session` before extending a stored chain, and falls through to the ceremony on a negative answer rather than surfacing an error.
