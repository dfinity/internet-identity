...
**Depends on:** `tracked-default-accounts.md`, whose account reference stores the session and whose principal index sits on the refresh path.

**Scope:** canister storage and API, plus the II frontend and its RPC surface. Breaks no existing app — the current RPC methods keep behaving exactly as they do.

## Context

When a user signs in to a dapp, II hands it a **delegation**: a canister-signed artifact naming the app's key and an expiry. The app chooses the lifetime through `maxTimeToLive`, up to `MAX_EXPIRATION_PERIOD_NS`, which is 30 days.
...
**Nothing revokes a delegation.** There is no lever to pull: rotating the salt changes what future derivations produce without touching an artifact already signed, so it would strand every existing principal while revoking nothing.

So "sign out" is a local gesture, and a leaked delegation is valid until it expires.
...
sequenceDiagram
    ...
    participant D as dapp canister
    U->>A: sign out
    A->>A: clears its own storage
    Note over A,II: II is not told, and could not act if it were
    U->>II: sign out everywhere?
    II--xA: no reach
    Note over D: a copy taken earlier still works<br/>for the rest of its 30 days
```
...
Split the one long-lived artifact in two: a **session** the canister holds, and a **short-lived app delegation** the app carries and re-mints from it.
...
sequenceDiagram
    ...
    participant D as dapp canister
    U->>IIF: ceremony, once
    IIF->>II: create a session on the account reference
    IIF-->>A: session chain (targets: II only)
    loop every 5 minutes
        A->>II: refresh, signed with the session chain
        II-->>A: app delegation, 5 minutes
        A->>D: calls
    end
    U->>II: revoke the session
    II--xA: refresh now fails
    Note over D: access ends within one<br/>delegation lifetime
```

The session is a record on the account reference, so it inherits the caps and eviction of `tracked-default-accounts.md`. The app never holds anything long-lived except a chain that is restricted to the II canister and revocable at any time.

**The shape already ships, scoped to MCP.** `mcp.rs` stores a grant, mints 5-minute delegations against it, and revokes by deleting it:
...
So the minting half is built and shipping. What MCP does not need, and apps do, is **many sessions per anchor**, somewhere to put them, and a way for a user to **see and revoke** them. MCP gets one session per anchor with a forward pointer from its config, which is why it needs neither an index nor a cap.

Two properties that are easy to miss:

- **A session cannot renew or clone itself.** Creating one requires an anchor access method, which a session does not have.
- **Refresh needs no browser.** The app calls the canister directly, so there is no navigation, popup, or iframe on the five-minute cadence.

### What this does not change

The initial ceremony, and anything for apps that have not upgraded their client. No existing method changes shape or behaviour.

### Cost

Shortening delegations does not grow the signature map: `SIGNATURE_EXPIRATION_PERIOD_NS` is one minute and `add_signature` prunes as it goes, so a signature is fetchable for a minute and the delegation the client keeps is the durable artifact. The cost is call volume, worked through in [cost and the TTL dial](#cost-and-the-ttl-dial).
...
| **Locator** | The `(anchor, application, account)` triple that identifies one account. What a derived principal resolves to through the principal index of `tracked-default-accounts.md` [session devices](#session-devices). |
...
| S2 | Ten sessions per account reference: reuse an unexpired session for the same locator and device, else prune expired and drop the least recently used. Creating a session never fails on the cap | Session cap |
| S3 | A row holding an unexpired session is not evictable | Further rules |
| S4 | Expired entries are pruned only when the list is written for another reason, so refresh never writes | Further rules |
| S5 | `session_seed = H(salt, "session", anchor, application, account, created_at, device_id)`, no allocator state | Session identity |
| S6 | A same-round collision for one account is a typed retryable error, not disambiguated | Collisions |
...
| S16 | Anchor-authenticated methods name sessions by locator, so the principal index is on the app-facing path only | Anchor-authenticated methods |
...
| S18 | Device registry is `StorableAnchor` field 7, capped at 20 with least-recently-used eviction, read through `identity_info` | Registry |
