...
**Scope:** Storage and canister bookkeeping. No Candid change to existing methods, no frontend change.

Three changes to the account reference list, each of which needs the others.

1. **Track default accounts** (§5), so the list records every account an anchor uses, not just named ones.
2. **Reap applications** (§8), so the origin registry stops growing. Tracking makes every sign-in at a new origin mint an application row, driven by an operation with no per-anchor cap, and without reaping nothing ever removes one. Reaping alone would be dead code, since eviction is the only thing that makes an application's reference count fall.
3. **Index accounts by derived principal** (§9), so II can resolve the caller behind an app delegation. This is only possible once the list is complete, and only affordable once idle entries are evicted.

All three maintain state derived from the same structure, so they share one write path (§4).
...
| **Evictable default** | Tracked default that is the only entry in its row, so deleting the whole row costs the anchor nothing. The unit the new cap counts (§7.1). A default sharing its row with named accounts is not evictable (§6.1). |
...
### 1.1 Default accounts are not tracked
...
### 1.3 There is no reverse index from a principal
...
1. Track every account an anchor uses at an application, default accounts included, bounded by a per-anchor cap resolved by evicting the least recently used tracked default.
2. Reap applications once no anchor references them.
3. Resolve a derived principal to its `(anchor, application, account)` triple, so that II can authorize a caller that presents an app delegation without being told who it is.

Eviction is non-destructive: it drops a timestamp, and the account is reconstructed at the same principal on next use.

---

## 3. The reference list becomes a three-state encoding
...
### 3.1 `read_account` must stop special casing the empty list
...
## 4. One write path for the reference list
...
| New in §5 (`set_account_last_used`) | One entry |
| New in §5.1 (`set_default_account_for_origin`) | One entry |
...
flowchart TD
    ...
    C --> D[anchor counters]
    C --> E["application counter, reap at 0 (§8)"]
    C --> F["principal index (§9)"]
```

Deletion is the same path with no current row, used by eviction (§6.2).
...
| Reference removed by a move (future) | no change | no change (§8.2) | update to the new owner |
| Reference mutated in place | no change | no change | re-insert if the value changed (§9.3) |

Two guards belong here rather than at the call sites:

- **Writing an empty list is rejected.** Under §3 an empty row is a tombstone, and no path in this design should ever produce one. Only the future move path may, so until then it is a programming error and must fail rather than silently deny an anchor its default account.
- **The salt must be present.** Index entries need `calculate_seed`, and `state::salt()` traps when unset (`state.rs:273-280`). See §9.4.

---

## 5. Tracking default accounts
...
This makes the reference-list row the single canonical marker of a relationship, which is what §8.1 counts. Without it, an application could be reaped while config rows survive, and those rows would be inherited by whatever claimed the number next.
...
Deliberately not "contains no `Some` references", which would also match the empty tombstone of §3.

Two properties follow:

- **Defaults are never evicted while a materialized account exists at that key.** Removing a `None` reference from a list holding named accounts lands in the third row state of §3, and the anchor loses its default account at that origin.
- **Eviction is an exact round trip.** Removing the row restores the absent state, which §3 defines as reconstructible. The next sign-in recreates it with `last_used = now`, at the same principal.

### 6.2 Remove the row, never empty it

Eviction deletes the row through the §4 path. It must never write back an empty vector: under §3 those two operations have opposite meanings, and an empty vector denies the anchor `anchor_seed(anchor, origin)` at that origin permanently. The §4 guard rejects empty writes so this cannot be expressed by accident, and it is still worth a dedicated test.
...
### 6.3 Selecting the victim

Range `(anchor, ApplicationNumber::MIN)..=(anchor, ApplicationNumber::MAX)`, the range `list_identity_account_references` (`storage.rs:1789`) already performs, and take the minimum `last_used` over rows satisfying §6.1, with `None` sorting oldest. That function discards the key in its `flat_map`, so a key-preserving variant is needed.

Cost is O(anchor's applications) reads, bounded by §7.3, paid only at the cap. Evicting a batch down to a watermark (90% of the cap) amortizes the scan across subsequent sign-ins.
...
Resolving a caller through the principal index also counts as usage and stamps `last_used` (§9.6), so an account actively making calls is never the victim.

---

## 7. Caps and counters

### 7.1 Two independent caps
...
That counts every default, evictable or not, so it is a cheap **upper bound** on the evictable count, taken from two fields that already exist. When it reaches the cap, the eviction scan (§6.3) yields the exact evictable count and the victim in the same pass, so the authoritative check costs nothing beyond what eviction already does. This is the shape `check_or_rebuild_max_anchor_accounts` (`account_management.rs:428`) already uses: trust the counter, recount when it claims the cap is hit, then act.

The bound is loosest for an anchor holding 500 non-evictable defaults, whose upper bound sits at the cap permanently so every new-origin sign-in triggers a scan. Such an anchor is by definition already at the materialized cap, and the scan is bounded by §7.3.

### 7.3 What an anchor can accumulate
...
The gap between the two totals is the reason §7.2's subtraction is an upper bound rather than the evictable count: it can read as high as 1000 while only 500 of those defaults are actually evictable.

Without moves, creating an account and holding one are the same event, so the single materialized cap does both jobs. A move feature splits them and has to keep an equivalent bound; see §10.
...
| `StorableApplication.stored_account_references` | Decrement on eviction. This is the reap predicate (§8.1) |
...
`rebuild_identity_account_counters` stays correct as written. With no move feature both anchor fields are still exactly derivable from the reference lists, so `check_or_rebuild_max_anchor_accounts` keeps self-healing for the materialized cap and for the §7.2 upper bound.

---

## 8. Reaping applications

### 8.1 The predicate is the existing reference counter

`StorableApplication.stored_account_references` is incremented once per reference created, in `update_counters`. Nothing has ever been removed, so its stored value is an accurate live count of references at that application today, not a historical sum (§1.2). No new field and no migration are needed; it only has to start being decremented, which §4 makes a property of one function rather than eight.

| Event | Effect |
| ----- | ------ |
| Reference created | `stored_account_references += 1` |
| Row removed by eviction (§6.2) | `stored_account_references -= 1` |
| Reaches 0 | Reap (§8.4) |
...
### 8.2 Tombstones must not decrement

A reference count and a row count agree everywhere except on the empty tombstone of §3, which holds zero references while its row is still alive. Left alone, that would let the counter reach 0 with a tombstone still present, and reaping would erase the tombstone and allow a moved-away default to be reconstructed at the same principal, which is precisely what the encoding exists to prevent.
...
With two anchors at the same application, a tombstone held by one keeps the count above zero while the other evicts, so the application is not reaped (§8.6).

### 8.3 The allocator must become monotonic
...
One entry per reference, so the steady-state size is the reference count, bounded per anchor by §7.3.

### 9.2 The map is injective
...
The one real risk is that a materialized default derives from `seed_from_anchor`, producing the same principal as the anchor's synthetic default at that origin. The two never coexist, because `create_default_account` replaces the `None` entry in place rather than appending. Once moves exist, what stops an anchor from acquiring a fresh `None` reference that collides with the account it gave away is the tombstone (§3). So the empty-row rule is load-bearing for this index too, and any future "tidy up empty rows" would corrupt the lookup as well as re-mint principals.

### 9.3 Maintenance, and the case the credential indexes do not have

The index is synced from the §4 diff, in the same shape as `sync_anchor_with_openid_credential_index` (`storage.rs:939`).
...
### 9.4 The salt is checked, not awaited
...
A sweep is preferred over lazy fill because it makes a lookup miss unambiguous. With lazy fill a miss would mean "unknown principal or not yet indexed", and §9.6 turns a miss into a denial.

### 9.6 What it is used for
...
- **A move in must backfill the recipient's own default reference**, exactly as `create_additional_account` does. Otherwise the recipient's row is `[Some(n)]` with no `None` reference, which §3 reads as "the default was moved away", and the recipient loses its own default account at an origin it gave nothing away at.
...
- A move that empties a row must not decrement the application counter (§8.2), and its index removal must be compare-and-delete (§9.3).
...
| Application growth is bounded, existing applications included | Reaping applies to legacy applications on the same predicate, with no migration (§8.5). Only applications held alive by a tombstone persist (§8.6) |
| The index roughly doubles the subsystem's footprint | One entry per reference, at a 29 byte key plus a small value, bounded per anchor by §7.3 |
...
| # | Decision | § |
| - | -------- | - |
| D1 | Reference list is a three-state encoding, absent is not empty | 3 |
| D2 | Delete the `is_empty()` synthetic-default branch in `read_account` | 3.1 |
| D3 | All reference-list writes go through one function that diffs previous against current and derives counters and index entries | 4 |
| D4 | That function rejects writing an empty list, which is reserved for the future move path | 4 |
| D5 | Eviction removes the row, never writes back an empty vector | 6.2 |
| D6 | Eviction predicate is `len() == 1 && [0].account_number.is_none()` | 6.1 |
| D7 | Defaults are never evicted while a materialized account exists at that key | 6.1 |
| D8 | The new cap is 500 evictable defaults per anchor, isolated from the 500 materialized cap. Sign-in never fails on it, since reaching it always means victims exist | 7.1 |
| D9 | `stored_account_references - stored_accounts` is the cheap upper bound on that cap; the eviction scan is authoritative | 7.2 |
| D10 | LRU victim found by on-demand scan with batch eviction to a watermark, not a stored pointer | 6.3 |
| D11 | `last_used: None` means never used and sorts oldest; neither creating a named account nor choosing a default stamps it | 6.4 |
| D12 | Eviction also removes the `AnchorApplicationConfig` row for the key | 6.2 |
| D13 | Tracked-default creation, eviction, and reaping are never archived | 11 |
| D14 | `set_default_account_for_origin` ensures a reference-list row exists, Invariant A | 5.1 |
| D15 | The reap predicate is the existing `StorableApplication.stored_account_references`, which is accurate today and only needs decrementing. No new field, no migration | 8.1 |
| D16 | A move that empties a row does not decrement; the count converts into the tombstone | 8.2 |
| D17 | Application numbers come from a monotonic cell at memory index 33 and are retired on reap, never reissued | 8.3 |
| D18 | Reaching zero removes the application row and the origin index entry, for legacy and new applications alike | 8.4, 8.5 |
| D19 | `update_counters` fails loudly instead of silently skipping when the application row is missing | 8.1 |
| D20 | The principal index maps derived principal to `(anchor, application, account)` at memory index 34 | 9.1 |
| D21 | The index diff compares values, not just keys, so materializing a default updates its entry in place | 9.3 |
| D22 | Index writes check the salt synchronously and error when it is unset. No await is added to any account mutation path | 9.4 |
| D23 | The index is populated by a cursor-driven batched backfill, sized by `internet_identity_total_account_references_count` | 9.5 |
| D24 | Resolution is `caller()`-only, never returns the anchor, and never accepts a principal argument | 9.6 |
| D25 | Resolving a caller stamps `last_used`, so an account in active use is never evicted | 9.6, 6.4 |
