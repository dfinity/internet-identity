...
This design writes a small entry the first time an identity uses an app, caps how many of
those an identity keeps, deletes app records nothing refers to any more, and indexes
accounts by the principal they derive to. Recording is cheap, and losing an entry costs little: the account is computed, so it returns at
the identical principal the next time the user signs in there. What a dropped entry does cost is
...
The two are independent in motivation and entangled in delivery. Recording use is what makes the list grow; the cap is what bounds it; deleting an app record is only reachable because the cap lets a count fall; and the index is only affordable once the thing it indexes is bounded. All four read and write the same entry, so they share one write path, which keeps the counts and the index consistent without every caller having to remember to update them.
...
Today a record only exists where a user named an account. We add one for the default account as well: the same kind of entry, with no name attached, holding when it was last used. It is a few bytes, against a whole account record, and it turns those entries into a complete list of the apps an identity has used.

The list could come from somewhere other than the canister. A browser could keep its own, which costs II nothing and answers the wrong question, because the user is asking where _they_ are signed in and a per-browser list cannot see the other devices. An app could declare itself, which makes the list depend on apps choosing to appear in a screen whose purpose is to catch the ones the user does not recognise. And usage could be appended to a log rather than folded into an entry, which answers "which apps" by reading the whole log and grows with visits rather than with apps. An entry per app is the answer already computed, and it is the only one of the four that a canister can render without trusting anybody.

### 2. Limit how many of those an identity can hold, and drop the ones it stopped using

Each identity gets a limit of 500 of these no-name entries. On reaching it, II deletes the ones used longest ago, down to 450 so that the work is spread across later sign-ins rather than repeated on every one.
...
1. **Leave it unbounded**, which is today's behaviour and survivable only because nothing creates these entries. Once a first sign-in does, the growth is driven by how many origins someone visits, and visiting origins is free.
2. **Expire by age.** An identity that uses forty apps twice a year loses all of them, and one that visits five thousand origins in a week keeps every entry. Age bounds how stale a record is, not how many there are, and storage is the thing under pressure.
...
Signing in stamps the entry as used, and so does a refresh, so an entry an application is actively calling from sits at the top of the order and is never the victim. What eviction reaches is idle entries, and recovery from a wrong guess is one sign-in.

One entry is never dropped: one that also holds a named account, since that account cannot be recomputed. An entry whose session is merely idle is not spared, and losing one signs that browser out of that app until the next sign-in. Sparing any entry with a session attached would instead let an identity sit above its limit for as long as anything kept a stale one alive.

### 3. Delete an app record once no identity refers to it

With entries now able to disappear, an app's count of referring accounts can reach zero, and the record can be removed along with the entry that maps its origin string to its internal number.

This is the one table no per-identity cap reaches. Approach 2 bounds what an identity holds and says nothing about a table shared by every identity and keyed by origin. Origins are free to create, so without removal that table grows with every distinct origin anybody has ever signed in at, and nothing ever gives an entry back. That is the growth this design would otherwise trade for the one it fixes.

Keeping app records forever was the alternative, and it means the table holds every origin any identity ever touched, including the ones visited once and the ones created to be visited once. The count is what makes removal reachable at all: without it, deciding whether an app is unreferenced means reading every identity's entries, where a count is maintained by the same write path that already has the entry in hand.
...
For every account that has an entry, II records which principal it derives to and which identity, app and account that principal means. This is the reverse direction the hash cannot narrow, bought once at write time instead of searched for at read time.
...
Today several call sites update the entries, the per-app counts and the origin map separately. They are consolidated into a
single function that takes the state an identity is to end up holding and works out the rest. The counts have to be
...
### Stage 3. Fix the read that treats an emptied entry as absent

An entry that has been emptied is not the same as one that never existed, and today the read path conflates
them.

### Stage 4. Start recording use, with the limit and the cleanup

This is the behavioural change: first use of an app writes an entry, the limit drops entries an identity stopped using,
and an app record with no remaining references is deleted.

### Stage 5. Maintain the principal map going forward

Every write to an entry also records the principal the account derives to. From this point the map is correct for anything
written after it, and incomplete for what came before.

### Stage 6. Fill in the map for accounts that already exist

A background sweep walks existing entries in batches, on a timer the canister starts for itself, adding the entries stage 5 could not know
