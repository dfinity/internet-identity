...
A user account is identified by a unique _Identity Anchor_, a natural number chosen by the canister.
...
Completes the registration of a new identity using OpenID Connect credentials. This is part of the identity registration flow that starts with `identity_registration_start`.

#### The `openid_credential_add` method

**Authorization**: This request must be sent to the canister with `caller` that is the self-authenticating id derived from any of the public keys of devices associated with the user before this call.

Adds a new OpenID Connect credential to an existing identity. The credential is identified by a JWT token and a salt value.
