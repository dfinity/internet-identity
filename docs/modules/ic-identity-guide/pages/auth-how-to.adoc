...
You can securely access applications running on the {IC} that use Internet Identity for authentication, by selecting an authentication method and having an Internet Identity ID number (user number) generated for you. You can create as many identities as you want by registering new ID numbers. 
