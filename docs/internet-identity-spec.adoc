...
2. The user is prompted for their user number
3. If no `DeviceData` with the purpose `#recovery` is found for the user number an error is displayed
...
The resulting `internet_identity.wasm` is ready for deployment on `mercury` (or any testnet if you want to run experiments).
...
==== Submitting proposal and voting on mercury

(This section was removed. We have deployed to mercury, and should not have to do it again. Please see git history if you need to know this.)
...
[source,bash]
----
dfx canister --network mercury --no-wallet info internet_identity
----


Once the deployment went through, tag the commit with `mercury-<date-from-proposal-url>`.
