...
=== Flow: Setup recovery
1. The user is offered two options for recovery
  * A security key
...
3. The device is added to the users account with the `is_recovery` flag set

=== Flow: Use recovery
1. On the login page the user selects "Recover my account"
2. The user is prompted for their user number
3. If no `DeviceData with` `is_recovery` is found for the user number an error is displayed
