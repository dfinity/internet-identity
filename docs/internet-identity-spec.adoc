 ...
 * Overview of the security and identity machinery, including the interplay of identities, pseudonyms, keys, and delegations
 * Interface as used by client applications frontends, i.e., our <<client-auth-protocol,client authentication protocol>>
 * The interface of the Internet Identity _backend_, i.e., describing its contract at the Candid layer, as used by its frontend
 * Important implementation notes about the Internet Identity backend
 * Internal implementation notes about the Internet Identity frontend
 * Notes about deployment

The Internet Identity consists of
...
Similarly, the client applications consist of a frontend (served by a canister) and (typically) one or more backend canisters. Only the frontend interacts with Internet Identity directly (via the <<client-auth-protocol,client authentication protocol>> described below).

== Goals, requirements and use cases

Internet Identity allows users to

 * maintain identities on the Internet Computer
 * log in with these identities using one out of a set of security devices
 * manage their set of security devices associated with an identity

Some functional requirements are

 * identities have separate pseudonyms per client application (more precisely, per client application frontend "hostname")
 * these pseudonyms are stable, i.e., do not depend on a user's security devices
 * the client frontends interact with any canister on the Internet Computer under the user’s pseudonym with that frontend
...
* The separate pseudonyms of a single user cannot be related merely based on their public key or principal ids, to impede user tracking.
* The security of the pseudonyms does not depend on the privacy of data stored on canisters, or transmitted to and from canisters. In particular, the delegations handed out by the backend canister must not be sensitive information.
* (many more, of course; apply common sense)

Some noteworthy security assumptions are:

* The delivery of frontend applications is secure. In particular, a user accessing the Internet Identity frontend through a TLS-secured HTTP connection cannot be tricked into running another web application.
+
...
* The security devices only allow the use of their keys from the same web application that created the key (in our case, the Internet Identity frontend).

* The data included in a redirect from the Internet Identity frontend to a client application backend can be considered confidential.

* For user privacy, we also assume the Internet Identity backend can keep a secret (but since data is replicated, we do not rely on this assumption for other security properties). 
...
NOTE: Due to replication of data in canisters, the salt should not be considered secret against a determined attacker. However, the canister will not reveal the salt directly and to the extent it is unknown to an attacker it helps maintain privacy of user pseudonyms.

A user identity is identified by a unique _ID number, a smallish natural number chosen by the canister.

A client application frontend is identified by its hostname (e.g., `abcde-efg.ic0.app`, `nice-name.ic0.app`, `non-ic-application.com`). Frontend applications can be served by canisters or by websites that are not hosted on the Internet
Computer.

An identity has a separate _pseudonym_ for each client application frontend (i.e., per hostname). This pseudonym is a https://docs.dfinity.systems/public/#id-classes[_self-authenticating id_] of the form
....
user_pseudonym = SHA-224(|ii_canister_id| · ii_canister_id · seed) · 0x02` (29 bytes)
....
...
....
seed = H(|salt| · salt · |id_number| · id_number · |frontend_host| · frontend_host)
....
where `H` is SHA-256, `·` is concatenation, `|…|` is a single byte representing the length of `…` in bytes, `id_number` is the ASCII-encoding of the ID number as a decimal number, and `frontend_host` is the ASCII-encoding of the client application frontend’s hostname (at most 255 bytes).

The Internet Identity backend stores the following data in user identities, indexed by the respective ID number:
...
When a client application frontend wants to log in as a user, it uses a _session key_ (e.g., Ed25519 or ECDSA), and by way of the authentication flow (details below) obtains a https://docs.dfinity.systems/public/#authentication[_delegation chain_] that allows the session key to sign for the user’s main pseudonym.

The delegation chain consists of one delegation, called the _client delegation_. It delegates from the user pseudonym (for the given client application frontend) to the session key. This delegation is created by the Internet Identity canister, and signed using a https://hydra.dfinity.systems/latest/dfinity-ci-build/ic-ref.pr-319/interface-spec/1/index.html#canister-signatures[canister signature]. This delegation is unscoped (valid for all canisters) and has a maximum lifetime of 8 days, with a default of 30 minutes.

The Internet Identity frontend also manages a _identity frontend delegation_, delegating from the security device’s public key to a session key managed by this frontend, so that it can interact with the backend without having to invoke the security device for each signature.

[#client-auth-protocol]
== Client authentication protocol

This section describes Internet Identity from the point of view of a client application frontend.
...
+
where the `userPublicKey` is the user’s pseudonym on the given frontend and `delegations` corresponds to the CBOR-encoded delegation chain as used for https://docs.dfinity.systems/public/#authentication[_authentication on the IC_].
...
[IMPORTANT]
--
The Internet Identity frontend will use `event.origin` as the “Frontend URL” to base the user pseudonym on. This includes protocol, full hostname and port. This means

* Changing protocol, hostname (including subdomains) or port will invalidate all user pseudonyms.
...
== The Internet Identity backend interface
...
The `register` method is used to create a new identity. The Internet Identity backend creates a _fresh_ ID number, creates the account record, and adds the given device as the first device.
...
In order to protect the Internet Computer from too many “free” update calls, and to protect the Internet Identity from too many user registrations, this call is protected using a proof of work obligation. The `register` call can only succeed if
...
The `add` method appends a new device to the given identity record.

The Internet Identity backend rejects the call if the identity already has a device on record with the given public key.

This may also fail (with a _reject_) if the user is registering too many devices for the specified identity.

*Authorization*: This request must be sent to the canister with `caller` that is the self-authenticating id derived from any of the public keys of devices associated with the identity before this call.

=== The `remove` method

The `remove` method removes a device, identified by its public key, from the list of devices associated with an identity.

It is allowed to remove the key that is used to sign this request. This can be useful for a panic button functionality.

It is allowed to remove the last key, to completely disable an identity. The canister may forget that identity completely then, assuming the ID number generation algorithm prevents new identities from getting the same ID number.
...
Fetches all data associated with an identity.

*Authorization*: Anyone can call this

=== The `get_principal` query method

Fetches the pseudonym for a given identity and front end.

*Authorization*: This request must be sent to the canister with `caller` that is the self-authenticating id derived from any of the public keys of devices associated with the identity before this call.

=== The `prepare_delegation` method

The `prepare_delegation` method causes the Internet Identity backend to prepare a delegation from the pseudonym associated with the given ID number and Client Application Frontend Hostname to the given session key.

This method returns the pseudonym that’s associated with the given Client Application Frontend Hostname. By returning this here, and not in the less secure `get_delegation` query, we prevent attacks that trick the user into using a wrong pseudonym.
...
== The Internet Identity backend internals

This section, which is to be expanded, describes interesting design choices about the internals of the Internet Identity canister. In particular
...
The primary data structure used by the backend is a map from ID number to the list of devices associated with the identity.
...
With the stable memory size of 4GiB we can store around 8 * 10^6 identity records in a single canister.
...
Idenity record for ID number N is stored at offset `sizeof(Header) + (N - user_number_range_lo) * sizeof(UserRecord)`.
...
NOTE: Even with that provision the same nonce can be used to ingress multiple messages, as `canister_inspect_message` is always run on a slightly old state. This cannot be avoided, but at least of these calls at most one can actually create a new identity.

=== Initialization

The Internet Identity canister is designed for sharded deployments.
There can be many simultaneously installed instances of the canister code, each serving requests of a subset of identities.
As identities are identified by their ID number, we split the range of ID number into continuous non-overlapping half-closed intervals and assign each region to one canister instance.
The assigned range is passed to the canister as an init argument, encoded in Candid:

....
type InternetIdentityInit = record {
  // Half-closed interval of ID numbers assigned to this canister, [ left_bound, right_bound )
...
== The Internet Identity frontend

The Internet Identity frontend is the user-visible part of Internet Identity, and where it all comes together. It communicates with
...
ID number, if known under the key `user_number`.
...
All update calls to the Internet Identity backend are made under the `device_identity` and are signed with the session key.
...
2. The frontend offers the choices
   * Welcome <ID number>. Do you want to log in?
...
6. 👆 The frontend creates a delegation from the security device key to the session key, and signs it with the security key, using any of the devices listed with the identity. It notes which device was actually used.
+
...
2. The frontend offers the choices
   * Create new identity 
   * Log into existing identity with existing device
   * Log into existing identity with new device
3. The user chooses to create a new identity
...
1. The frontend notices that no `user_number` is present in local storage.
   (Or user said “log in as different identity” in returning flow.)
2. The frontend offers the choices
   * Create new identity 
   * Log into existing identity with existing device
   * Log into existing identity with new device
3. The user selects “Log into existing identity with existing device”
4. The frontend asks the user for their ID number, and stores that in `user_number`.
...
2. The frontend offers the choices
   ...
   * Log into existing identity with existing device
   * Log into existing identity with new device
3. The user selects “Log into existing identity with new device”
4. The frontend asks the user for their ID number, and stores that in `user_number`.
...
- `userNumber` is the ID number, as a decimal number
...
3. The device is added to the identity with the purpose `#recovery` set

=== Flow: Use recovery
1. On the login page the user selects "Recover my account"
2. The user is prompted for their ID number 
3. If no `DeviceData` with the purpose `#recovery` is found for the ID number an error is displayed
...
6. The front end calls `get_principal()` to obtain the user- and front-end-specific pseudonym.
7. The user is asked if they want to log into the client application, showing the client application frontend’s hostname and the used pseudonym.
...
3. If this is the last device of the user, then the front end will not delete the device and instead abort.
...
* how the Internet Identity canister id stays predictable and well-known
