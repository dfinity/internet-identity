...
- `AccountInfo` is per-*account*; this is per-*application*. Conflating them confuses the model.
...
`set_default_account` and `get_default_account` remain unchanged. `set_anchor_application_config` is allowed to set or clear *either* field; the two never need to be set atomically.
...
> **`AnchorApplicationConfig` holds preferences that change *identity behavior* per application — not UI chrome.**
