...
3. The frontend asks the user for its user id, and stores that in `userid`.
4. The frontend generates a link to be opened on another device where an existing authentication device exists.
+
TODO: Specify the link format here
5. On other device’s frontend: Extract `userid`, `publicKey` and `credentialid` from link
6. On other device’s frontend: If no `identity` is present, `lookup` identities and pick suitable one.
