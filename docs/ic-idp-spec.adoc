...
* The frontend knows the current list of backend canisters, and thus which canister the user’s record is on.
...
7. 👆 The frontend creates a delegation from the security device key to the session key, and signs it with the security key, using any of the devices listed in the user account. It notes which device was actually used.
8
