 ...
 * the frontend, a web application served by an otherwise stateless canister with a _well-known_ canister id, and
...
Each backend canister exclusively controls the identies of a set of users.
...
A backend canister maintains a secret salt (in the following the `salt`), a 32 byte long blob that is obtained via the Internet Computer’s source of secure randomness.
...
When a client application frontend wants to log in as a user, it uses a _session key_ (e.g., Ed25519 or ECDSA), and by way of the OAUTH protocol (details below) obtains a https://docs.dfinity.systems/public/#authentication[_delegation chain_] that allows the session key to sign for the user’s identity.

This delegation chain consists of one delegation, called the _client delegation_. It delegates from the user’s identity (for the given client application frontend) to the session key. This delegation is created by the Internet Identity Service backend canister responsible for this user, and signed using a https://hydra.dfinity.systems/latest/dfinity-ci-build/ic-ref.pr-319/interface-spec/1/index.html#canister-signatures[canister signature]. This delegation is unscoped (valid for all canisters) and has a lifetime of *TODO*.
...
As state (backed by stable memory to be upgrade safe), the index canister maintains a list of `BackendInfo`, ordered by their user number. This data also includes, for each canister, the hash of the wasm module installed there.

The index canister also has, as static data, the wasm module of the backend canister.

TODO: Insights/logging (e.g. installation/upgrade failures, ongoing processes).
...
* The index canister checks if the last canister is more than 50% full (using `stats`), and if so, creates and installs a new canister.
+
The new canister is assigned the next range, as obvious.
+
Because of the asynchronous nature of canister installation, it must set a flag to not do this multiple times in parallel. The canister must be written defensively to clear this flag if installation fails (e.g. `ic0.call_on_cleanup`)
+
Once `install_call` comes back, it can include the canister in the respond to `backends()`.

* For each canister with a wasm module hash that does not match the hash of the wasm module, an upgrade (without stopping) is performed.
+
Like above, this needs to be guarded with a flag “in process” and defensive handling of failures.
