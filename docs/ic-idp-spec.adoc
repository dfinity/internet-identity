...
* the `publicKey` contains the public key of the session key pair.
* the `maxTimetoLive` indicates the desired time span until the requested delegation should expire. The Identity Provider frontend is free to set an earlier expiry time, but should not create a larger.
--
5. Now it expects a message back, with data `event`.
6. If `event.origin !== "https://identity.ic0.app"`, ignore this message.
7. The `event.data` value is is a JS object with the following type:
+
 interface InternetIdentityAuthResponse {
   message: "authorize-client-success";
   delegations: [{
     delegation: {
       pubkey: ArrayBuffer;
       expiration: bigint;
...
+
 interface InternetIdentityAuthResponse {
   message: "authorize-client-failure";
   text: string;
 }
+
...
service : {
  ...
  prepare_delegation : (UserNumber, FrontendHostname, SessionKey, maxTimetoLive : nat64) -> (UserKey, Timestamp);
...
The expiration timestamp is determined by the backend, but no more than  `maxTimetoLive` nanoseconds in the future.
