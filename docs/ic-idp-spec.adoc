...
All the integers (u64, u32, u16) are encoded in Big-Endian.
....
Storage ::= {
  Header
  UserRecords
...
User record for user number N is stored at offset `sizeof(Header) + N - user_number_range_lo`.
...
We use pre-upgrade hook to serialize the header and post-upgrade hook to deserialize it back.
