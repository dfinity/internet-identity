//! Types for the shared inbound-DKIM challenge primitive.
//!
//! Both the recovery flow ([`crate::email_recovery`]) and the
//! verified-emails flow ([`crate::verified_email`]) build on the same
//! ownership-proof primitive: the user emails a DKIM-signed message
//! carrying a canister-issued nonce to a known mailbox; the canister
//! verifies DKIM + DMARC and matches the verified `From:` against the
//! claimed address. This module owns the request/response types,
//! polling status, and error/diagnostics surface that the primitive
//! exposes — flow-neutral, keyed by nonce, dispatched to the right
//! anchor sink by the canister-side `PendingKind`.
//!
//! Trust comes from one of two paths, picked by the canister per call:
//!
...
//!    pipeline. See `docs/ongoing/email-recovery.md` §8.4 / §8.5 for
//!    the rationale (no selector probing — the selector is
//!    authoritative from the email itself).
...
pub enum EmailChallengeError {
    ...
    /// Failed shape validation. Distinct from `DomainNotSupported`,
    /// which is about a valid address whose domain can't be verified.
    InvalidEmailAddress(String),
    /// Per-bucket cap reached (currently only the verified-emails bucket).
    LimitReached {
        limit: u8,
    },
