...
///
/// Some fields, like `openid_google`, have a nested `EnabledOrDisabled<>`, this indicates
/// enable/disable status (e.g. `EnabledOrDisabled(None)` disables a feature while `None` leaves it untouched).
