...
mod session_revocation_tests {
    ...
    fn signing_out_a_browser_with_nothing_to_revoke_writes_nothing() {
        ...
        let removed = storage
            .revoke_browser_sessions(anchor_number, 9, TEST_NOW)
            .unwrap();
...
mod write_path_property_tests {
        ...
        match rng.below(7) {
            ...
            1 => {
                ...
                if let Some(mut account) = storage.read_account(&AccountKey {
                    ...
                    let _ = storage.write_account(account, TEST_NOW);
                }
            }
            2 => {
                let account_number = pick_account(storage, anchor_number, &origin, rng);
                let _ =
                    storage.set_default_account(anchor_number, origin, account_number, TEST_NOW);
            }
            3 => {
            ...
            5 => {
                if let Some(key) = pick_session(storage, anchor_number, &origin, rng) {
                    let _ = storage.revoke_session(&key, TEST_NOW);
                }
            }
            _ => {
                let _ =
                    storage.revoke_browser_sessions(anchor_number, rng.below(4) as u32, TEST_NOW);
            }
        }
    }
    ...
    fn pick_session(
        ...
        origin: &str,
        rng: &mut Rng,
    ) -> Option<crate::storage::account::SessionLocator> {
        let application_number =
            storage.lookup_application_number_with_origin(&origin.to_string())?;
        ...
        Some(crate::storage::account::SessionLocator {
            anchor_number,
            origin: origin.to_string(),
            account_number,
            session_id,
...
/// Keeping these out of the heap is the whole reason for the map, and the deadline
/// order has to come back with them or the sweep and the next batch to send both
/// read the wrong end of the queue.
#[test]
