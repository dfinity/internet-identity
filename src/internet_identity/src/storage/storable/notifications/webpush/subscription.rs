...
///
/// The browser's `p256dh` and `auth` keys are absent because the push carries
/// no body: they exist only to encrypt an RFC 8291 payload.
...
impl Storable for StorableWebPushSubscription {
    ...
    // Subscription (~1100 bytes: endpoint ≤ 1 KiB + a 65-byte key + anchor +
    // timestamp) plus the embedded JWT pool (~2000 bytes: 30 signatures of 64
    // bytes + timestamp), CBOR-encoded. Worst case measured at 3112 by
