...
pub fn enter_device_registration_mode(
    ...
    id: Option<ValidatedRegistrationId>,
    ...
    state::tentative_device_registrations_mut(|registrations| {
        state::lookup_tentative_device_registration_mut(|lookup| {
            ...
            match registrations.entry(identity_number) {
                Entry::Occupied(_) => Err(AuthnMethodRegistrationModeEnterError::AlreadyInProgress),
                Entry::Vacant(entry) => {
                    let expiration = time() + REGISTRATION_MODE_DURATION;
                    if let Some(reg_id) = &id {
                        lookup.insert(reg_id.clone(), identity_number);
                    }
    ...
    state::tentative_device_registrations_mut(|registrations| {
        state::lookup_tentative_device_registration_mut(|lookup| {
            ...
            let registration = registrations
                ...
                .ok_or(TentativeDeviceRegistrationError::DeviceRegistrationModeOff)?;
            ...
            match registration {
                ...
                // Else return error
                _ => Err(TentativeDeviceRegistrationError::AnotherDeviceTentativelyAdded),
            }
        })
    })
}
...
async fn new_confirmation_code() -> DeviceConfirmationCode {
    ...
    let rand = u32::from_be_bytes(res[..4].try_into().unwrap_or_else(|_| {
        trap(&format!(
            "when creating device confirmation code from raw_rand output, expected raw randomness to be of length 32, got {}",
            res.len()
        ));
    }));
