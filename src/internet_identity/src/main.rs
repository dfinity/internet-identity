...
async fn webpush_subscribe_device(
    ...
    notifications::webpush::subscription::subscribe_device(
        anchor_number,
        endpoint,
        p256dh.into_vec(),
        auth.into_vec(),
...
async fn notification_grant_consent(
    ...
    notifications::consent::grant_consent(anchor_number, origin, account_number).await
}
...
fn initialize(maybe_arg: Option<InternetIdentityInit>) {
    ...
    notifications::delivery::schedule_dispatch();
}
