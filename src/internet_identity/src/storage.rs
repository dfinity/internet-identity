...
const WEBPUSH_SEAL_MEMORY_INDEX: u8 = 40u8;
const NOTIFICATION_SENDER_CANISTERS_MEMORY_INDEX: u8 = 41u8;
...
/// `sender_canister -> StorableSenderOrigin`. Which origin a canister may send
/// notifications for, cached from the dApp's well-known file at consent time.
const NOTIFICATION_SENDER_CANISTERS_MEMORY_ID: MemoryId =
    MemoryId::new(NOTIFICATION_SENDER_CANISTERS_MEMORY_INDEX);
...
pub struct Storage<M: Memory> {
    ...
    notification_sender_canisters_memory_wrapper: MemoryWrapper<ManagedMemory<M>>,
    pub(crate) notification_sender_canisters_memory:
        StableBTreeMap<Principal, StorableSenderOrigin, ManagedMemory<M>>,
...
impl<M: Memory + Clone> Storage<M> {
    ...
    fn init_with_header(memory: M, header: Header) -> Self {
        ...
        let notification_sender_canisters_memory =
            memory_manager.get(NOTIFICATION_SENDER_CANISTERS_MEMORY_ID);
        ...
        Self {
            ...
            notification_sender_canisters_memory_wrapper: MemoryWrapper::new(
                notification_sender_canisters_memory.clone(),
            ),
            notification_sender_canisters_memory: StableBTreeMap::init(
    ...
    fn sync_session_index(
        ...
        for (principal, (handle, account)) in &after {
            ...
            // account numbers changes, which a sign-in does not. Without this a session at
            // a list that predates the index resolves to nothing, and nothing else would
            // put it right.
    ...
    pub fn memory_sizes(&self) -> HashMap<String, u64> {
        HashMap::from_iter(vec![
            ...
            (
                "notification_sender_canisters_memory".to_string(),
                self.notification_sender_canisters_memory_wrapper.size(),
            ),
            (
