...
use crate::storage::storable::notifications::backlog::{
    BacklogRow, StorableBacklogEntry, StorableBacklogKey,
};
...
const NOTIFICATIONS_BACKLOG_MEMORY_INDEX: u8 = 38u8;
...
impl<M: Memory + Clone> Storage<M> {
    ...
    fn init_with_header(memory: M, header: Header) -> Self {
        ...
        Self {
            ...
            notifications_backlog_memory_wrapper: MemoryWrapper::new(
    ...
    pub fn drain_backlog_notifications(
        ...
        let parked: Vec<_> = self.notifications_backlog_memory.iter().collect();
        for (key, _) in &parked {
            self.notifications_backlog_memory.remove(key);
        }
