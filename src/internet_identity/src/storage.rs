...
impl<M: Memory + Clone> Storage<M> {
    ...
    pub fn create_session(
        ...
        // the ones given up above, the identity's session count, the account reference
        // list this origin gets if it did not have one, and any tracked default the write
        // pushes over its own cap — all of it follows from this, and a refusal, the cap
        // included, leaves none of it behind.
    ...
    /// Ends one session, index entry and count included, and reports whether it was there.
    ///
    ...
    pub fn revoke_session(&mut self, key: &SessionRecordKey) -> Result<bool, StorageError> {
        ...
        let mut anchor = self.read(key.anchor_number)?;
        ...
        if !write
            ...
            .sessions
            .iter()
            .any(|session| session.session_id == key.session_id)
        {
            return Ok(false);
        }
        write
