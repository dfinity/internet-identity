...
/// Monotonic `ApplicationNumber` allocator. Reaped numbers are retired rather than
/// reissued, so the number cannot be derived from how many applications exist.
...
impl<M: Memory + Clone> Storage<M> {
    ...
    /// Existing application numbers are dense from zero, so the number of application
    /// rows is the first free number.
    ///
    /// Seeding here rather than at the first allocation is what keeps that true:
    /// reaping shrinks the row count, so a reap that lands before the first allocation
    /// would otherwise seed the allocator onto a live number.
    ...
    /// * `Err` if writing the reference list back failed, which is a bookkeeping
    ///   fault rather than a missing account, so the two are not collapsed.
    ...
    /// application. Derived counters follow from the list, so they are diffed here
    /// rather than maintained by callers.
    fn write_reference_list(
        ...
        // Resolved before anything is written. A canister keeps the state a rejected
        // call already wrote, so a check that runs mid-write leaves the row and the
        // counters disagreeing, which is exactly the drift the reap predicate cannot
        // survive.
        let application = self
            .stable_application_memory
            .get(&application_number)
