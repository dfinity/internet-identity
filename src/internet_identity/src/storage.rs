...
const NOTIFICATION_SENDER_BINDINGS_MEMORY_INDEX: u8 = 41u8;
...
impl<M: Memory + Clone> Storage<M> {
    ...
    /// Per subscribed device of `anchor_number` that can receive a push for
    /// `origin_hash`, the inputs to build one Web Push: `(endpoint,
    /// vapid_public_key, sealed_blob, jwt_pool)`: the relay URL to POST to, the
    /// VAPID key and JWT pool that authorise the request, and the sealed payload
    /// that forms the body. A device is skipped if it lacks a seal for this
    /// origin or a JWT pool, since neither can be delivered.
    pub fn devices_for_delivery(
        &self,
        anchor_number: AnchorNumber,
