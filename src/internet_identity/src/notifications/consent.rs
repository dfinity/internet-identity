...
use super::{authorize_query, authorize_update, ensure_enabled, feature_enabled, validate_origin};
...
fn list_consented_origins(anchor_number: AnchorNumber) -> Vec<FrontendHostname> {
    let range_start = (anchor_number, StorableOriginSha256::MIN);
    let range_end = (anchor_number, StorableOriginSha256::MAX);
    storage_borrow(|storage| {
        storage
            .notifications_consent_memory
...
pub fn consent_status(
    ...
    if super::push::subscription::has_any_subscription(anchor_number) {
...
pub fn consented_origins(anchor_number: AnchorNumber) -> Vec<FrontendHostname> {
    ...
    list_consented_origins(anchor_number)
}
...
mod tests {
    ...
    fn grant_then_revoke_consent_round_trips() {
        ...
        assert_eq!(list_consented_origins(anchor), vec![origin.clone()]);
        ...
        assert!(!has_consent(anchor, origin.clone()));
        assert!(list_consented_origins(anchor).is_empty());
        assert_eq!(
            storage_borrow(|s| s.notifications_recipient_index_memory.get(&recipient)),
            None
        );
