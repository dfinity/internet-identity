...
mod tests {
    use super::*;
    use crate::delegation::{calculate_account_seed_with_salt, calculate_anchor_seed};
    ...
    fn an_added_account_is_addressed_by_its_own_principal() {
        ...
        let added = storage_borrow_mut(|storage| {
            storage
                .create_account(anchor, origin.clone(), "second".to_string(), 1_000)
        ...
        assert_eq!(
            added_seed,
            calculate_account_seed_with_salt(&crate::state::salt(), account_number, &origin),
