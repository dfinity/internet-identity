...
/// The origin this sender canister is authorized to notify for, learned from
/// that origin's well-known senders list (see `cache_sender`). `None` if the
/// canister was never cached, i.e. it is not an authorized sender for anyone.
pub fn authorized_origin(sender: Principal) -> Option<FrontendHostname> {
