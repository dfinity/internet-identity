...
use super::super::{
    authorize_query, authorize_update, check_enabled, feature_enabled, NotificationError,
};
...
/// What a device has signed, so the frontend knows when to top up. The pool is
/// spent by elapsed time — the signature for a window is picked by index and
/// never removed — so this reports how many windows the pool covers and when it
/// started, not a count of unused signatures. A count would sit at 30 forever
/// and the frontend's top-up would never fire.
#[derive(candid::CandidType, serde::Deserialize, Clone, Copy, Debug, PartialEq, Eq)]
pub struct JwtPoolStatus {
    /// Windows this pool covers, from `issued_at_ns`.
    pub pool_len: u32,
...
fn replace_jwt_pool(
    ...
    validate_jwt_pool(&signatures)
        .map_err(|err| NotificationError::InvalidSubscription(vec![err]))?;
...
/// How many windows the stored pool covers, and when it was minted.
pub fn jwt_pool_state(anchor_number: AnchorNumber, endpoint: String) -> Option<JwtPoolStatus> {
    ...
    jwt_pool_status(anchor_number, &endpoint).map(|(pool_len, issued_at_ns)| JwtPoolStatus {
        pool_len,
        issued_at_ns,
    })
}
