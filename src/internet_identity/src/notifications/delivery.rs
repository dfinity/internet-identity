...
fn take_pending_deliveries(max: usize, now_ns: Timestamp) -> Vec<Delivery> {
    ...
    while deliveries.len() < max {
        ...
        fan_out_to_devices(&notification, now_ns, &mut deliveries);
    }
    deliveries
}

/// Turns one buffered ping into per-device sends. For each of the user's devices
/// that has a sealed payload for this origin and a VAPID JWT still valid for its
/// relay, appends a ready-to-POST `Delivery`; a device missing either has
/// nothing sendable, so it is skipped.
fn fan_out_to_devices(
    ...
    let rows =
        storage_borrow(|storage| storage.webpush_deliveries_for_origin(anchor, &origin_hash));
...
fn push_headers(delivery: &Delivery) -> Vec<(String, String)> {
