...
test('derive Ed25519 via SLIP 0010', async () => {
  await Promise.all(
    testVectorsSLIP10.map(async (testVector, i) => {
      ...
      let identity = await ed25519.fromSeedWithSlip0010(seedBlob, testVector.derivationPath);

      const keyPair = identity.getKeyPair();
      expect(keyPair.secretKey.slice(0, 32)).toEqual(expectedPrivateKey);
      expect(keyPair.publicKey).toEqual(Ed25519PublicKey.fromRaw(expectedPublicKey));
    }),
  );
});
