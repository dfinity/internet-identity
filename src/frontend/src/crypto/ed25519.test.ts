...
test("Can derive identity from invalid mnemonic", async () => {
  await expect(ed25519.fromMnemonicWithoutValidation("")).resolves.not.toThrow();
  await expect(ed25519.fromMnemonicWithoutValidation("g4rb4g3")).resolves.not.toThrow();
  await expect(ed25519.fromMnemonicWithoutValidation("basket actual")).resolves.not.toThrow();
});
