...
test('cubehash("hello")', () => {
  expect(Buffer.from(cubeHash(Buffer.from('hello', 'utf8'))).toString('hex'))
...
test('cubehash("")', () => {
  expect(Buffer.from(cubeHash(Buffer.from('', 'utf8'))).toString('hex'))
    .toBe("44c6de3ac6c73c391bf0906cb7482600ec06b216c7c54a2a8688a6a42676577d");
});

test('cubehash("The quick brown fox jumps over the lazy dog")', () => {
  expect(Buffer.from(cubeHash(Buffer.from('The quick brown fox jumps over the lazy dog', 'utf8'))).toString('hex'))
