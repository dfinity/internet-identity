 ...
 */
export async function fromSeedWithSlip0010(masterSeed: Uint8Array, derivationPath?: number[]): Promise<Ed25519KeyIdentity> {
    let [slipSeed, chainCode] = await generateMasterKey(masterSeed);

    if (derivationPath) {
        for (let i = 0; i < derivationPath.length; i++) {
            [slipSeed, chainCode] = await derive(slipSeed, chainCode, derivationPath[i] | HARDENED);
        }
    }
 ...
 * e.g. to generate m/44'/223’/0’/0’/0' the derivation path should be [44, 223, 0, 0, 0]
 */
export async function fromMnemonic(mnemonic: string, derivationPath?: number[]): Promise<Ed25519KeyIdentity> {
    const seed = mnemonicToSeedSync(mnemonic);
    return fromSeedWithSlip0010(seed, derivationPath);
}

async function generateMasterKey(seed: Uint8Array): Promise<[Uint8Array, Uint8Array]> {
...
async function derive(parentKey: Uint8Array, parentChaincode: Uint8Array, i: number): Promise<[Uint8Array, Uint8Array]> {
