...
export const questions = {
  ...
  invalidSeedphrase: {
    ...
    answer: html`
    <p>Most likely, it means that the recovery phrase (also known as seed phrase) is not BIP39 compatible.</p>

    <p>A recovery phrase generated by Internet Identity will contain an anchor number followed by some words. For instance, "135033 squirrel soccer ...". The number is the anchor for which the recovery phrase is generated; note that Internet Identity will strip the number before trying to recover an Identity, meaning the number is only meant for humans!</p>

    <p>What Internet Identity will check, on the other hand, is that the remainder (the seed phrase without the anchor) is BIP39-compatible. Do note that some BIP39 backup solutions only allow storing the first 4 letters in accordance with the BIP39 protocol. While BIP39 was designed such that only using the first 4 letters of each word, Internet Identity will not "re-inflate" the words for you, and you may need to do that in order to recover your anchor.</p>
        `,
    links: [
      {
        name: "BIP39",
...
function renderQuestion(faq: Question) {
  ...
  >
    ...
    <div>
      <p class="faq__answer">${faq.answer instanceof TemplateResult ? faq.answer : html`<p>${faq.answer}</p>`}</p>
      ${faq.links !== undefined && faq.links.length > 0 ? renderFaqLinks(faq.links) : ""}
    </div>
  </li>`;
}
