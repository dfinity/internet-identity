...
export const login = async (): Promise<{ userId: bigint, connection: IDPActor }> => {
    try {
      ...
      await confirm("An unexpected error occured during login. Please try again", err);
...
const init = async (userId: bigint): Promise<LoginResult> => {
    return new Promise(resolve => {
        ...
        loginButton.onclick = async (ev) => {
            ...
            ev.stopPropagation();
            try {
                resolve({ tag: "ok", userId, connection: await IDPActor.reconnect(userId) })
            } catch (err) {
                resolve({ tag: "err", message: `Failed to login as ${userId}`, detail: err.toString() })
            }
        }
