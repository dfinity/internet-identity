...
console.log(
  "sample login_hint: ",
  blobToHex(Ed25519KeyIdentity.generate().getKeyPair().publicKey.toDer())
);
...
async function generate_access_token(
  ...
  const res = await idp_actor.get_delegation(userId, identity);
  if (!isDelegationResponse(res)) {
    throw Error("got bad variant?");
  }
  const { delegation } = res;
  // session key is taken from login_hint query parameter instead of
  // being generated by the root identity
  ...
  const rootChain = DelegationChain.fromDelegations(
    [rootToDeviceDelegation],
    derBlobFromBlob(blobFromUint8Array(Uint8Array.from(publicKey)))
  );
  const sessionChain = await DelegationChain.create(
    identity,
    sessionKey,
    new Date(Date.now() + THREE_DAYS),
    { previous: rootChain }
  );
  console.log("root to device delegation");
  console.log(rootToDeviceDelegation);

  console.log("device to session delegation");
  console.log(rootToDeviceDelegation);
  // A prompt so I have time to inspect console.log
  prompt("An unnecessary prompt so that I can inspect the console logs.");
  return JSON.stringify(sessionChain.toJSON());
}
