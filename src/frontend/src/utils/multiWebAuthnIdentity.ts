...
import {
  ...
  export type CredentialData = { pubkey : DerEncodedBlob, credentialId : CredentialId };
  ...
  export class MultiWebAuthnIdentity extends SignIdentity {
     ...
     */
    public static fromCredentials(credentialData: CredentialData[]) : MultiWebAuthnIdentity {
    ...
    public async sign(blob: BinaryBlob): Promise<BinaryBlob> {
      ...
      this.credentialData.forEach(cd => {
        if (cd.credentialId.equals(blobFromUint8Array(Buffer.from(result.rawId)))) {
          // would be nice if WebAuthnIdentity had a directly usable constructor
          const strippedKey = cd.pubkey.slice(19);
          this._actualIdentity = WebAuthnIdentity.fromJSON(JSON.stringify({
