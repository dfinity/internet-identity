...
describe("lastUsedIdentitiesStore", () => {
  ...
  describe("updateLastUsedIdentity", () => {
    const newName = "Updated Name";
    const newCredId = strToUint8Array("new-cred-id");
    const newAuthMethod = { passkey: { credentialId: newCredId } };
    const newCreatedAtMillis = 1695000000000;

    beforeEach(() => {
      // Add an identity to be updated
      lastUsedIdentitiesStore.addLastUsedIdentity({
        identityNumber: identity1,
        name: name1,
        authMethod: { passkey: { credentialId: credId1 } },
    ...
    it("should not update if the identity does not exist", () => {
      const nonExistentIdentity = BigInt("999");
      const originalState = get(lastUsedIdentitiesStore).identities;

      lastUsedIdentitiesStore.updateLastUsedIdentity(nonExistentIdentity, {
        name: "A New Name",
      });

      expect(get(lastUsedIdentitiesStore).identities).toEqual(originalState);
    });
    ...
    it("should update only the createdAtMillis of an existing identity", () => {
      lastUsedIdentitiesStore.updateLastUsedIdentity(identity1, {
