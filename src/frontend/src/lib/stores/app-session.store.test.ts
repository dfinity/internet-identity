...
describe("app session store", () => {
  ...
  it("keeps accounts of one identity apart", async () => {
    ...
    await storeAppSession(
      { identityNumber, accountNumber: BigInt(3), origin: ORIGIN },
      record(anHourFromNow()),
    );

    const listed = await appSessionsForOrigin(ORIGIN);
    expect(listed).toHaveLength(2);
    expect(listed.map((entry) => entry.accountNumber)).toContain(undefined);
    expect(listed.map((entry) => entry.accountNumber)).toContain(BigInt(3));
  });
