 ...
 *
 * The pool is spent by elapsed time, not by use: the signature for a window is
 * picked by index and never removed, so the canister can only report how many
 * windows the pool holds and when it started. Deriving what is left from those
 * two is the whole reason `JwtPoolStatus` reports them instead of a count — a
 * count never decreased, so the top-up below never fired and every browser went
 * quiet about thirty days after subscribing.
 */
