// The device half of the VAPID pool: generate a P-256 keypair, sign one JWT per validity
// window, and hand II only the raw signatures. II holds no VAPID key and reassembles
// each compact JWT from a signature at send time (see the canister's vapid_jwt module).
// The header bytes, the claim order and WINDOW_NS are a wire contract with that module.
 ...
 *
 * The pool is spent by elapsed time, not by use: a window's signature is picked by index
 * and never removed, so the canister reports the pool's length and start time and
 * what is left is derived from those rather than counted.
 */
export const windowsRemaining = ({
  poolLen,
  issuedAtNs,
  nowNs,
}: {
  ...
  const elapsedNs = nowNs - issuedAtNs;
  const spent = elapsedNs <= BigInt(0) ? 0 : Number(elapsedNs / WINDOW_NS);
  return Math.max(0, poolLen - spent);
};
...
/**
 * Generates a fresh VAPID keypair. Split from {@link signJwtPool} because subscribing
 * needs the public key and yields the endpoint the pool is signed over.
 */
...
/**
 * base64url of the JWT claims for one window, the exact bytes the canister reassembles
 * and the relay verifies. `exp = issued_at + (window + 1) * 24h` in seconds, keys
 * ordered aud, exp, sub.
 */
