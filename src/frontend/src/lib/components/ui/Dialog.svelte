<script lang="ts">
  ...
  // When navigating while a dialog is open, close the dialog first and
  // let it transition out before proceeding with the navigation.
  let isOpen = $state(true);
  let pendingNavigationUrl: URL | undefined;

  beforeNavigate((navigation) => {
    if (!isOpen || pendingNavigationUrl !== undefined) return;
