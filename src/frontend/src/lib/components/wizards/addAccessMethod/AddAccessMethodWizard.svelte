<script lang="ts">
  ...
  let continueOnAnotherDevice = $state(false);
  ...
  const handleContinueOnAnotherDevice = () => {
    continueOnAnotherDevice = true;
  };
</script>

<Dialog {onClose}>
  {#if continueOnAnotherDevice}
  ...
  {:else if addAccessMethodFlow.view === "addPasskey"}
    <AddPasskey
      ...
      continueOnAnotherDevice={handleContinueOnAnotherDevice}
      {isUsingPasskeys}
    />
  {/if}
