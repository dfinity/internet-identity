...
webauth1PK = toPublicKey webauthSK
webauth1ID :: EntityId
...
tests wasm_file = testGroup "Tests" $ upgradeGroups $
  ...
  , withoutUpgrade $ iiTest "register with wrong user fails" $ \cid -> do
    getChallenge cid dummyUserId >>= callIIReject cid dummyUserId #register . (device1, invalidPOW,)
  , withoutUpgrade $ iiTest "register with bad pow fails" $ \cid -> do
    getChallenge cid dummyUserId >>= callIIReject cid dummyUserId #register . (device1, invalidPOW,)
  , withoutUpgrade $ iiTest "register with future pow fails" $ \cid -> do
    getChallenge cid dummyUserId >>= callIIReject cid dummyUserId #register . (device1, powAt cid (20*60*1000_000_000),)
  , withoutUpgrade $ iiTest "register with past pow fails" $ \cid -> do
    setCanisterTimeTo cid (20*60*1000_000_000)
    getChallenge cid dummyUserId >>= callIIReject cid dummyUserId #register . (device1, powAt cid 1,)
