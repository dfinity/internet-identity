...
tests wasm_file = testGroup "Tests" $ upgradeGroups $
  ...
  , withUpgrade $ \should_upgrade -> testCase "upgrade from stable memory backup" $ withIC $ do
    ...
    lookupIs cid 10_030 [#alias .== "dfx" .+ #credential_id .== Nothing .+ #pubkey .== dfxPK,#alias .== "testkey" .+ #credential_id .== Nothing .+ #pubkey .== webauthPK .+ #purpose .== "authentication" .+ #key_type .== "unknown"]
