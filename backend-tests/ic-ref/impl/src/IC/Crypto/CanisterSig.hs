...
genPublicKey (EntityId cid) seed = BS.toLazyByteString $
...
parsePublicKey = first T.pack . runGetLazy do
    t <- getWord8
...
genSig cert tree = toLazyByteString $ encodeTerm $ TTagged 55799 $ TMap
...
parseSig s = do
    ...
    certificate <- parseField "certificate" kv >>=
      parseBlob "certificate" >>= decodeCert
