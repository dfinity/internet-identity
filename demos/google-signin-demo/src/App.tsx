...
const transport = new PostMessageTransport({
  url: "http://localhost:5173/authorize?openid=google",
});
...
const App: React.FC = () => {
  ...
  const handleSignIn = async () => {
    try {
      const publicKey = (await ECDSAKeyIdentity.generate())
        .getPublicKey()
        .toDer();
      ...
      const [result, attributesResponse] = await Promise.all([
        result_promise,
        attribute_promise,
      ]);
      ...
      console.log(
        "Attribute result",
        JSON.stringify(attributesResponse, null, 2),
      );
